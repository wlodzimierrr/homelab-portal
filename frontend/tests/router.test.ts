...
describe('service route helpers', () => {
  it('recognizes the current service details route', () => {
    assert.equal(isServiceDetailsPath('/services/api-gateway'), true)
  ...
  it('recognizes the current service deployments route', () => {
    ...
    assert.equal(isServiceDetailsPath('/services/api-gateway/deployments'), false)
    assert.equal(getServiceIdFromPath('/services/api-gateway/deployments'), 'api-gateway')
  })
})

describe('resolveAppRoute', () => {
  it('resolves service details routes before falling back', () => {
