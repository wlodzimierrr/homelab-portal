...
# ---------------------------------------------------------------------------
# Add-to-project generators (add a service to an existing project namespace)
# ---------------------------------------------------------------------------
...
class ScaffoldAddServiceInput:
    ...
    project_id: str
    service_name: str  # component suffix, e.g. "worker" → service_id = "{project_id}-worker"
    description: str
    owner_email: str
    owner: str
    namespace: str
...
def validate_add_service(
    ...
    """Validate that the new service can be added without conflicts."""
    ...
    # Check for duplicate resource files in the base kustomization
    is_db = inp.template in _DB_TEMPLATES
    if is_db:
        check_resources = [
...
def generate_gitops_add_service_files(
    inp: ScaffoldAddServiceInput,
) -> tuple[dict[str, str], list[str]]:
    """Generate files for adding a service to an existing project.

    Returns (new_files, new_base_resources) where new_base_resources is the
    list of filenames to append to the project's base kustomization.yaml.
    """
    service_id = inp.service_id
    validate_service_name(service_id)
    ...
    # --- Service account ---
    files[f"{base_prefix}/serviceaccount-{inp.service_name}.yaml"] = _render_template(
        """
        apiVersion: v1
        kind: ServiceAccount
        metadata:
    ...
    if is_db:
        # Database service: StatefulSet + Service + Secret
        db_port = int(t["db_port"])  # type: ignore[arg-type]
        db_engine = str(t["db_engine"])
        container_name = str(t.get("container_name", t.get("db_image", "").split(":")[0]))
        _generate_add_service_db_files(
            files, new_resources, inp, base_prefix,
            db_port, db_engine, container_name, service_id,
        )
    else:
        # Application service: Deployment + Service
        container_port = int(t["container_port"])  # type: ignore[arg-type]
        service_port = int(t["service_port"])  # type: ignore[arg-type]
        container_name = str(t["container_name"])
        health_path = str(t["health_path"])
        ...
        _generate_add_service_app_files(
            files, new_resources, inp, base_prefix,
            container_port, service_port, container_name,
            health_path, readiness_path, observability_mode,
            service_id,
        )

        # Overlay patches (dev + prod) — only for app services
        for env_name in ("dev", "prod"):
            _generate_add_service_overlay_files(
                files, inp, env_name, container_name, service_id,
            )
...
def _generate_add_service_app_files(
    ...
    """Generate deployment, service, and optional servicemonitor for an app component."""
    probes = _render_template(
        """
        readinessProbe:
          httpGet:
    ...
    # --- Service ---
    files[f"{base_prefix}/{inp.service_name}-service.yaml"] = _render_template(
        """
        apiVersion: v1
        kind: Service
        metadata:
    ...
    # --- ServiceMonitor (app-native only) ---
    if observability_mode == "app-native":
        files[f"{base_prefix}/servicemonitor-{inp.service_name}.yaml"] = _render_template(
            """
            apiVersion: monitoring.coreos.com/v1
            kind: ServiceMonitor
            metadata:
...
def _generate_add_service_db_files(
    ...
    """Generate StatefulSet, Service, and Secret for a database component."""
    if db_engine == "postgres":
        env_block = _render_template(
            """
            env:
              - name: POSTGRES_USER
                valueFrom:
    ...
    # --- Secret ---
    files[f"{base_prefix}/{inp.service_name}-credentials-secret.yaml"] = _render_template(
        """
        apiVersion: v1
        kind: Secret
        metadata:
        ...
        project_id=inp.project_id,
        service_name=inp.service_name,
    ).rstrip() + "\n" + _indent_block(secret_data.rstrip(), 2) + "\n"
    ...
    statefulset_lines.extend(_indent_block(env_block.rstrip(), 10).splitlines())
...
def _generate_add_service_overlay_files(
    ...
    """Generate per-env overlay patch for the new service's deployment."""
    if env_name == "prod":
        replicas, cpu_req, mem_req, cpu_lim, mem_lim = "2", "100m", "128Mi", "500m", "512Mi"
    else:
        replicas, cpu_req, mem_req, cpu_lim, mem_lim = "1", "50m", "64Mi", "300m", "256Mi"
    ...
    files[f"{env_prefix}/patch-{inp.service_name}-deployment.yaml"] = _render_template(
        """
        apiVersion: apps/v1
        kind: Deployment
        metadata:
...
def build_catalog_add_service_entry(
    existing_services_yaml: str,
    inp: ScaffoldAddServiceInput,
) -> str:
    ...
    display_name = " ".join(
        word.capitalize() for word in inp.project_id.split("-")
    ) + f" {inp.service_name.capitalize()}"
    repo_url = inp.repo_url or inp.workloads_repo_url
    observability_mode = str(_TEMPLATES[inp.template]["default_observability_mode"])
...
def update_overlay_kustomization_patches(existing: str, new_patch_path: str) -> str:
    """Insert a new patch path into an existing overlay kustomization.yaml."""
    if f"path: {new_patch_path}" in existing:
    ...
    for line in lines:
        ...
        if in_patches and stripped.startswith("- path:"):
            ...
            continue
        if in_patches:
            # End of patches block — flush sorted
            patch_lines.append(f"- path: {new_patch_path}")
    ...
    if in_patches:
        # patches was last block in file
        patch_lines.append(f"- path: {new_patch_path}")
        patch_lines = sorted(dict.fromkeys(patch_lines))
        for p in patch_lines:
            rebuilt.append(f"  {p}")
